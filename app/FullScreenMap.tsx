...
function PolylineTooltip({ start, selected }: { start: any; selected: any }) {
    // Prefer airport codes when available in selected.attendee_routes[start.name]
    // Each route hop is expected to contain an airport code field like
    // airport_code, airportCode or code. Fall back to city names when missing.
    const startName = start.name || "";
    const selectedName = selected.event_location || "";
    let displayStart = startName;
    let displaySelected = selectedName;
    try {
        const routesForStart = (selected as any)?.attendee_routes?.[start.name];
        if (Array.isArray(routesForStart) && routesForStart.length > 0) {
            const firstHop = routesForStart[0];
            const lastHop = routesForStart[routesForStart.length - 1];
            const getCode = (hop: any) => (hop?.airport_code || hop?.airportCode || hop?.code || "").toString().toUpperCase();
            const firstCode = getCode(firstHop);
            const lastCode = getCode(lastHop);
            if (firstCode) displayStart = firstCode;
            if (lastCode) displaySelected = lastCode;
        }
    } catch (e) {
        // ignore and fall back to names
    }
    ...
    return (
        <>
            <div className="flex justify-center items-center gap-4 text-black">
                <div className="font-semibold text-sm ">{displayStart}</div>
                {isZurichGeneva ? (
